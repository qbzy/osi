...
// Структура Allocator (заменена на CustomAllocator) — условно доступна из внешних реализаций
typedef struct CustomAllocator CustomAllocator;

// Прототипы функций из внешних реализаций аллокатора
...
// Фallback-функции для системного mmap/munmap (вместо system_alloc/system_free)
...
void fallback_free(CustomAllocator* dummy, void* ptr) {
    (void)dummy;
    // Здесь для примера освобождаем фиксированный размер
    // (в реальном коде нужно знать точный размер).
    munmap(ptr, 4096);
}
...
// Тест №1: Множественное выделение и освобождение (ранее bulk_test → теперь alloc_test)
...
// Тест №2: Проверка слияния (раньше coalescing_test → теперь merge_test)
